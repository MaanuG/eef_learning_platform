...
function Overview() {
  ...
  const statItems = [
    { icon: '⏳', label: 'Pending Approvals', value: stats.pending, color: '#d97706', bg: '#fffbeb', trend: 'Needs attention' },
  ...
  return (
    <div>
      {/* Hero */}
      <div className="hero-card mb-6" style={{ marginBottom: 24 }}>
        <div style={{ position: 'relative', zIndex: 1 }}>
          <p style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>Admin Dashboard</p>
          <h2 style={{ fontSize: 32, color: 'white', marginBottom: 6, letterSpacing: '-0.02em' }}>Good morning! 👋</h2>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>
            {stats.pending > 0 ? `You have ${stats.pending} pending approval${stats.pending > 1 ? 's' : ''} waiting.` : 'Everything is up to date.'}
          </p>
          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
            <button className="btn btn-primary btn-sm" onClick={() => navigate('/admin/approvals')}>View Approvals</button>
            <button className="btn btn-sm" style={{ background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.8)', border: '1px solid rgba(255,255,255,0.12)' }} onClick={() => navigate('/admin/classrooms')}>Manage Classrooms</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid-3 mb-6" style={{ marginBottom: 24 }}>
        {statItems.map((s, i) => (
          <div key={i} className="stat-card" style={{ background: `linear-gradient(135deg, white 60%, ${s.bg})` }}>
      ...
      <div className="grid-2" style={{ gap: 20 }}>
        {/* Pending Approvals */}
        <div className="card">
          <div className="flex-between mb-4" style={{ marginBottom: 18 }}>
            <div>
              <h3 style={{ fontSize: 17, letterSpacing: '-0.01em' }}>Pending Approvals</h3>
              <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{stats.pending} waiting for review</p>
            </div>
          ...
          {pending.length === 0 ? (
            ...
            <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: '1px solid #f1f5f9' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <span className={`badge ${u.role === 'educator' ? 'badge-teal' : 'badge-blue'}`} style={{ marginTop: 3, fontSize: 10 }}>
        ...
        <div className="card">
          <div className="flex-between mb-4" style={{ marginBottom: 18 }}>
            <div>
              <h3 style={{ fontSize: 17, letterSpacing: '-0.01em' }}>Recent Classrooms</h3>
              <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{stats.classrooms} total</p>
            </div>
          ...
          {classrooms.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '28px 0', color: '#94a3b8' }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>🏫</div>
              <p style={{ fontSize: 13 }}>No classrooms yet</p>
            </div>
          ) : classrooms.map(c => (
...
function Approvals() {
  ...
  const approve = async (id) => { await api.post(`/users/${id}/approve`).catch(() => {}); setPending(pending.filter(u => u.id !== id)); };
  const reject = async (id) => { await api.post(`/users/${id}/reject`).catch(() => {}); setPending(pending.filter(u => u.id !== id)); };

  return (
    <div>
      <div className="page-header">
        <h2>Account Approvals</h2>
        <p>{pending.length} request{pending.length !== 1 ? 's' : ''} pending review</p>
      </div>
      {pending.length === 0 ? (
        <div className="card empty-state"><div className="empty-state-icon">✅</div><p style={{ fontSize: 15, fontWeight: 600 }}>All caught up!</p><p style={{ fontSize: 13, marginTop: 4 }}>No pending approvals</p></div>
      ) : (
        <div className="card">
          {pending.map(u => (
            <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 0', borderBottom: '1px solid #f1f5f9' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <span className={`badge ${u.role === 'educator' ? 'badge-teal' : 'badge-blue'}`} style={{ marginTop: 5 }}>
                  {u.role === 'educator' ? '👩‍🏫 Educator' : '🎓 Student'}
                </span>
              </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 0', borderBottom: '1px solid #f8fafc' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {u.status === 'rejected' && (
                <button className="btn btn-primary btn-sm" onClick={() => reactivate(u.id)}>Reactivate</button>
              )}
            </div>
          </div>
        ))}
...
function Classrooms() {
  ...
  useEffect(() => {
    Promise.all([api.get('/classrooms/'), api.get('/users/educators'), api.get('/users/students')])
      .then(([c, e, s]) => { setClassrooms(c.data); setEducators(e.data); setStudents(s.data); })
      .catch(() => {});
  }, []);

  const createClass = async () => {
    const r = await api.post('/classrooms/', newClass).catch(() => {});
    if (r) { setClassrooms([...classrooms, r.data]); setShowCreate(false); setNewClass({ name: '', subject: '', description: '', color: '#0d9488' }); }
  };

  const assignStudent = async (classId, studentId) => { await api.post(`/classrooms/${classId}/assign-student`, { user_id: parseInt(studentId) }).catch(() => {}); api.get('/classrooms/').then(r => setClassrooms(r.data)).catch(() => {}); };
  const assignEducator = async (classId, educatorId) => { await api.post(`/classrooms/${classId}/assign-educator`, { educator_id: parseInt(educatorId), is_admin: false }).catch(() => {}); api.get('/classrooms/').then(r => setClassrooms(r.data)).catch(() => {}); };

  return (
    <div>
      <div className="flex-between mb-6" style={{ marginBottom: 24 }}>
        <div className="page-header" style={{ margin: 0 }}>
          <h2>Classrooms</h2>
          <p>{classrooms.length} total classrooms</p>
        </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {colors.map(c => (
                  <button key={c} onClick={() => setNewClass({...newClass, color: c})} style={{ width: 30, height: 30, borderRadius: 8, background: c, border: newClass.color === c ? '3px solid #1e293b' : '3px solid transparent', cursor: 'pointer', transition: 'transform 0.1s' }} />
                ))}
              </div>
            </div>
            <button className="btn btn-primary btn-lg" style={{ width: '100%', justifyContent: 'center' }} onClick={createClass} disabled={!newClass.name}>
              Create Classroom
            </button>
          </div>
        </div>
      ...
      {showManage && (
        <div className="modal-overlay" onClick={() => setShowManage(null)}>
          <div className="modal" style={{ maxWidth: 580 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              <label className="form-label">Add Educator</label>
            ...
            <div style={{ fontSize: 13, color: '#64748b', display: 'flex', gap: 20 }}>
              <div><strong style={{ color: '#0f1923' }}>Educators:</strong> {showManage.educator_count}</div>
              <div><strong style={{ color: '#0f1923' }}>Students:</strong> {showManage.student_count}</div>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Admin access to all {classrooms.length} classrooms</p>
      </div>
      <div className="grid-3">
        {classrooms.map(c => (
